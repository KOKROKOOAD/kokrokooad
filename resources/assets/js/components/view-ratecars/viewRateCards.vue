        ...
        <div class="col-lg-8">
          <div class="page-header-title">
            <div class="d-inline">
              ...
              <span>
                What type of rate card do you want to
                <code>View</code>, Select a rate card.
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="row">
        <div class="col-sm-12">
            ...
            <div class="card-block animated fadeIn">
              <div class="row">
                <div class="col-md-12 col-sm-12 col-xl-12 m-b-30">
                  ...
                  <p
                    class="text-danger"
                    v-show="ratecards.length == 0"
                  >No rate cards found for selected media house</p>
                  <select
                    v-show="ratecards.length > 0 "
                    name="select"
                    class="form-control form-control-primary"
                    v-model="card_title_id"
                    @change="getSelectedRatedCard(card_title_id)"
                  >
                    <option disabled value selected>Select Ratecard</option>
                    <option
                      v-for="(card,index) in ratecards"
                      :value="card.rate_card_title_id"
                      :key="index"
                    >{{card.rate_card_title}}</option>
                  </select>
                </div>
          ...
          <view-card-details
            ...
            :weekdays="weekdays"
            :weekends="weekends"
            :viewWeekendSegments="viewWeekendSegments"
            :days_of_week="days_of_week"
            :days_of_weekend="days_of_weekend"
            :view_rate_card="view_rate_card"
            :view_rate_card_w="view_rate_card_w"
            :media="medi"
...
export default {
  ...
  methods: {
    fetchRateCardTitles() {
      let self = this;
      axios
        .get("fetchratecard/api/" + self.getSelectMedia)
        .then(function(res) {
          self.process = false;
          self.view = true;
